...
async def login_for_access_token(request: Request, db: SessionDep, form: LoginRequest):
    ...
    if not user:
        ...
        return html_templates.body_response(
            request, "login.html", "Login", {"error": error}
        )
    ...
    response = html_templates.landingpage_response(
...
def logout(request: Request):
    ...
    response = html_templates.body_response(
        request, "login.html", "Login", {"request": request, "msg": msg}
    )
